...
class GameManager:
    ...
    def reset_game(self):
        ...
        for i in range(self.num_players):
            ...
            attempts = 0
            while attempts < 10 and any(pos in occupied_positions for pos in new_snake.body):
                new_snake = Snake(self.board.width, self.board.height)
                attempts += 1
    ...
    def step(self, action):
        """Single player step function (preserved for backward compatibility)"""
        if self.num_players > 1:
    ...
    def handle_apple_collision(self, snake_index):
        ...
        if snake.head in self.get_current_apple_coords():
            apple = next(apple for apple in self.apples if apple.position == snake.head)
    ...
    def step_two_snakes(self, action_1, action_2):
        """Legacy method for two-snake game mode, now redirects to the generalized method"""
