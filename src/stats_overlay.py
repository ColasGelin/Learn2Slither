...
class StatsOverlay:
    ...
    def _calculate_stats(self):
        ...
        self.stats = {
            ...
            len(self.scores),
            'max_score':
            int(np.max(scores_array)),
            'min_score':
            int(np.min(scores_array)),
            'mean_score':
            ...
            self.training_duration /
            len(self.scores) if len(self.scores) > 0 else 0,
            'scores_10':
            int(np.sum(scores_array >= 10)),
            'scores_20':
            int(np.sum(scores_array >= 20)),
            'scores_30':
            int(np.sum(scores_array >= 30)),
            'scores_40':
            int(np.sum(scores_array >= 40)),
            'scores_50':
            int(np.sum(scores_array >= 50)),
            'improvement_rate':
            self._calculate_improvement_rate(scores_array),
            'last_100_avg':
            float(np.mean(scores_array[-100:]))
            if len(scores_array) >= 100 else float(np.mean(scores_array))
        }
    ...
    def _calculate_improvement_rate(self, scores_array):
        ...
        return (
            (last_quarter_avg - first_quarter_avg) / first_quarter_avg) * 100
    ...
    def draw(self, screen):
        ...
        # Title
        title_text = self.font_large.render("TRAINING STATISTICS - PLAYER 1",
                                            True, self.GREEN)
        ...
        # Score
        section_text = self.font_medium.render("SCORE ", True, self.BLUE)
        ...
        performance_stats = [
            f"Total Episodes: {self.stats['ttl_episodes']}",
        ...
        achievement_stats = [
            f"Scores >= 10: {self.stats['scores_10']} "
            f"({self.stats['scores_10']/self.stats['ttl_episodes']*100:.1f}%)",
            f"Scores >= 20: {self.stats['scores_20']} "
            f"({self.stats['scores_20']/self.stats['ttl_episodes']*100:.1f}%)",
            f"Scores >= 30: {self.stats['scores_30']} "
            f"({self.stats['scores_30']/self.stats['ttl_episodes']*100:.1f}%)",
            f"Scores >= 40: {self.stats['scores_40']} "
            f"({self.stats['scores_40']/self.stats['ttl_episodes']*100:.1f}%)",
            f"Scores >= 50: {self.stats['scores_50']} "
            f"({self.stats['scores_50']/self.stats['ttl_episodes']*100:.1f}%)",
        ]
        ...
        # Training TIme
        section_text = self.font_medium.render("TRAINING TIME", True,
                                               self.BLUE)
        screen.blit(section_text, (col2_x, col2_y))
        col2_y += 35
        ...
        time_stats = [
            ...
            f"{self._format_time(self.stats['training_duration'])}",
            f"Episodes per Hour: {3600/self.stats['avg_episode_duration']:.0f}"
            if self.stats['avg_episode_duration'] > 0 else
            "Episodes per Hour: N/A",
            f"Average Episode Duration: "
            f"{self._format_time(self.stats['avg_episode_duration'])}",
        ]
        ...
        instruction_text = self.font_small.render(
            "Press 'H' to return to replay | Press 'Q' to quit", True,
            self.RED)
